...
void accelCIC(const double *box, int N, double boxlen, double x, double y,
              double z, double *a) {

    double fac = boxlen / N;

    a[0] = 0.0;
    a[0] -= gridCIC(box, N, boxlen, x + 2 * fac, y, z);
    a[0] += gridCIC(box, N, boxlen, x + 1 * fac, y, z) * 8;
    a[0] -= gridCIC(box, N, boxlen, x - 2 * fac, y, z) * 8;
    a[0] += gridCIC(box, N, boxlen, x - 2 * fac, y, z);
    a[0] /= 12 * fac;

    a[1] = 0.0;
    a[1] -= gridCIC(box, N, boxlen, x, y + 2 * fac, z);
    a[1] += gridCIC(box, N, boxlen, x, y + 1 * fac, z) * 8;
    a[1] -= gridCIC(box, N, boxlen, x, y - 2 * fac, z) * 8;
    a[1] += gridCIC(box, N, boxlen, x, y - 2 * fac, z);
    a[1] /= 12 * fac;

    a[2] = 0.0;
    a[2] -= gridCIC(box, N, boxlen, x, y, z + 2 * fac);
    a[2] += gridCIC(box, N, boxlen, x, y, z + 1 * fac) * 8;
    a[2] -= gridCIC(box, N, boxlen, x, y, z - 2 * fac) * 8;
    a[2] += gridCIC(box, N, boxlen, x, y, z - 2 * fac);
    a[2] /= 12 * fac;
}
